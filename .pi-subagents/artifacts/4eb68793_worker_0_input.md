...
- 创建 applyCodexEditor(ctx) 函数: ctx.ui.setEditorComponent((tui, theme, keybindings) => new CodexStyleEditor(...))
- cursorOpen 用 ctx.ui.theme.getFgAnsi("accent")
- 导出 restoreDefaultEditor(ctx): ctx.ui.setEditorComponent(undefined)
- default export: session_start 时调用 applyCodexEditor(ctx)
...
- 导出 applyStatusline(ctx) 和 restoreDefaultFooter(ctx) 函数
...
- default export function(pi) 注册命令
...
给文件末尾加: `export default function() {}`
先读文件确认末尾内容再追加。
...
- 完成后验证: 每个 .ts 文件都有 export default, 语法正确 (node --check 或目视检查)
